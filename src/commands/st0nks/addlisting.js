      ...
      message.channel.awaitMessages(filter, {
        ...
        msg.edit(`<:astolfoHappy:683035783963082797> **Okiiiiii, how many would you trade?**\n*(The quantity of ${currency} you wish to exchange)*\n[Examples: \`1\`, \`1000\` and \`1337\`]`).then(msg => {
          message.channel.awaitMessages(filter, {
            ...
            if (quantity < 1) return message.channel.send(`u cant trade less dannnnnn 1 ${currency}!!!`)
            ...
            collected.first().delete();
              ...
              message.channel.awaitMessages(filter, {
                ...
                if (quantity2 < 1) return message.channel.send(`bruh u cant recieve less than 1 ${currency2}...`)
                collected.first().delete();
                msg.delete();
                message.delete();
                
                //var lmfaofixpls = await eco.fetchBalance(message.author.id);
                
                //if (lmfaofixpls.Balance < quantity && currency == "credit") return message.channel.send(`u don't haz ${quantity} ${currency}!!!!`)
                
                if (currency == "credit") {
                  eco.SubtractFromBalance(message.author.id, quantity)
                }
                
                if (currency == "apocrypha token") {
                  if (userData.get(`u${message.author.id}.at`) < quantity) {
                    message.channel.send(`u no has ${quantity} ${currency}!!!!!`)
                    return;
                  }
                  userData.subtract(`u${message.author.id}.at`, quantity)
                }
                
                if (currency == "trapcoin") {
                  if (userData.get(`u${message.author.id}.tc`) < quantity) {
                    message.channel.send(`u no hav ${quantity} ${currency}`)
                    return;
                  }
                  userData.subtract(`u${message.author.id}.tc`, quantity)
                }
